...
public class InvocationServiceImpl implements InvocationService {
    ...
    @Override
    public Object invokeFunction(List<String> arguments, boolean logEnabled) {
        try {
                    ...
                    new Thread(() -> {
                        try {
                            completableFuture.complete(Optional.ofNullable(functionMethod.invoke(function, range(0, arguments.size()).boxed()
                                            .map(i -> jsonParser.parseString(arguments.get(i), functionMethod.getParameters()[i].getParameterizedType()))
                                            .toArray()))
                                    .orElse(""));
